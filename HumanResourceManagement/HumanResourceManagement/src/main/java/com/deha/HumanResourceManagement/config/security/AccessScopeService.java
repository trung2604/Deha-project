...
public class AccessScopeService {
    ...
    public boolean isOfficeManager(User user) {
        return user != null && user.getRole() == Role.MANAGER_OFFICE;
    }

    public boolean isDepartmentManager(User user) {
        return user != null && user.getRole() == Role.MANAGER_DEPARTMENT;
    }
    ...
    public void assertCanManageOffice(UUID targetOfficeId) {
        ...
        if (!isOfficeManager(actor)) {
            throw new ForbiddenException("Only admin or office manager can manage office-scoped data");
        }
        UUID actorOfficeId = actor.getOffice() != null ? actor.getOffice().getId() : null;
        if (actorOfficeId == null || !actorOfficeId.equals(targetOfficeId)) {
            throw new ForbiddenException("Office manager can only manage data in their own office");
        }
    }
    ...
    public void assertCanManageDepartment(UUID targetDepartmentId) {
        ...
        if (actorDepartmentId == null || !actorDepartmentId.equals(targetDepartmentId)) {
    ...
    public void assertCanAccessUser(User targetUser) {
        ...
        if (isDepartmentManager(actor)) {
            UUID actorDeptId = actor.getDepartment() != null ? actor.getDepartment().getId() : null;
            UUID targetDeptId = targetUser.getDepartment() != null ? targetUser.getDepartment().getId() : null;
            if (actorDeptId == null || !actorDeptId.equals(targetDeptId)) {
