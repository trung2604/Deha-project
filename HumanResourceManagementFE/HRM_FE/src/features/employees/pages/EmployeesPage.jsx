  ...
  const handleSave = async (employee) => {
    try {
      if (editingEmployee) {
        ...
        setEmployees((prev) => prev.map((e) => (e.id === employee.id ? (updated ?? employee) : e)));
        toast.success('Employee updated successfully');
      } else {
        const created = await employeeService.createEmployee(employee);
        const newEmployee = created ?? employee;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span style={{ fontSize: '14px', fontWeight: '500' }}>Add Employee</span>
        </button>
      </div>
